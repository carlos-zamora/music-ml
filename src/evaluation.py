...
def train_epoch(model, loader, optimizer, criterion, device=None, on_batch=None):
    ...
    model.train()
    total_loss = 0.0
    for x, artist_idx, artist_mask, bpm, key_feat, y in loader:
        x, batch_size, num_partitions = _reshape_partition_batch(x)
        x, artist_idx, artist_mask, bpm, key_feat, y = x.to(device), artist_idx.to(device), artist_mask.to(device), bpm.to(device), key_feat.to(device), y.to(device)
        ...
        logits = model.classify(audio_feat, artist_idx, bpm, key_feat, artist_mask)
