...
def train_epoch(model, loader, optimizer, criterion, device=None, on_batch=None):
    ...
    model.train()
    total_loss = 0.0
    for x, artist_idx, artist_mask, bpm, key_feat, panns_feat, y in loader:
        x, batch_size, num_partitions = _reshape_partition_batch(x)
        x, artist_idx, artist_mask, bpm, key_feat, panns_feat, y = (
            x.to(device), artist_idx.to(device), artist_mask.to(device),
...
def run_kfold_evaluation(model_factory, ds, config=None, device=None, collate_fn=None):
    ...
    from panns_features import PANNS_VERSION as _PANNS_VERSION
    summary = {
        "$schema": "./summary.schema.json",
        "git_commit": get_git_commit(),
        "panns_version": _PANNS_VERSION,
        "config": {
            "n_splits": config.n_splits,
