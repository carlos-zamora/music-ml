...
def test_classify_with_mask_shape(tiny_model, tiny_model_inputs):
    ...
    panns_feat  = tiny_model_inputs["panns_feat"]
    logits = tiny_model.classify(audio_feat, artist_idx, bpm, key_feat, panns_feat, artist_mask)
...
def test_classify_without_mask_shape(tiny_model, tiny_model_inputs):
    ...
    key_feat   = tiny_model_inputs["key_feat"]
    panns_feat = tiny_model_inputs["panns_feat"]
    logits = tiny_model.classify(audio_feat, artist_idx, bpm, key_feat, panns_feat)
    assert logits.shape == (4, 3)
...
def test_unknown_key_and_bpm_accepted(tiny_model, tiny_model_inputs):
    # All-zero key_feat (unknown), bpm (unknown), and panns (unknown) should produce valid logits
    from panns_features import PANNS_DIM
    audio_feat = torch.randn(4, 128)
    artist_idx = torch.zeros(4, 1, dtype=torch.long)
    bpm        = torch.zeros(4, 2)
    key_feat   = torch.zeros(4, 4)
    panns_feat = torch.zeros(4, PANNS_DIM + 1)
    logits = tiny_model.classify(audio_feat, artist_idx, bpm, key_feat, panns_feat)
    assert logits.shape == (4, 3)
